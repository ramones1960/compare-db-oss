...
// トレードオフ スコアデータ（各軸: 書き込み速度 / 読み取り速度 / クエリ柔軟性 / 水平スケール / 整合性 / 運用容易さ）
const DB_SCORES = {
...
// 各評価軸・各スコア段階の判定基準（1〜5）
const SCORE_CRITERIA = {
  "書き込み速度": [
    "1: <1K ops/sec — WALオーバーヘッド大・同期書き込み主体",
    "2: 1K〜10K ops/sec — 標準的なRDB書き込み（バッファ書き込み）",
    "3: 10K〜100K ops/sec — バッチ書き込み・非同期コミット活用",
    "4: 100K〜1M ops/sec — LSMツリー or インメモリ書き込み",
    "5: >1M ops/sec — インメモリ主体 or append-only で事実上無制限",
  ],
  "読み取り速度": [
    "1: >100ms — 複雑なインデックス探索・フルスキャンを含む",
    "2: 10〜100ms — 通常のRDBインデックス参照",
    "3: 1〜10ms — 最適化済みインデックス・ページキャッシュ活用",
    "4: 0.1〜1ms — インメモリ or NVMe SSD最適化",
    "5: <0.1ms — インメモリ・ハッシュテーブル直引き（例: Redis）",
  ],
  "クエリ柔軟性": [
    "1: 完全一致のみ — KV参照・プライマリキー検索のみ",
    "2: 範囲検索・基本フィルタ — 集計は限定的",
    "3: 基本JOIN・GROUP BY・集計関数",
    "4: ウィンドウ関数・サブクエリ・全文検索・地理空間など",
    "5: フルSQL + 特殊クエリ（グラフ/ベクトル/時系列専用構文等）",
  ],
  "水平スケール": [
    "1: シングルノードのみ — スケールアウト不可",
    "2: Read Replicaのみ — 書き込みはシングルマスター",
    "3: 手動シャーディング可能 — 運用コスト高",
    "4: 自動シャーディング・再バランス対応",
    "5: クラスタネイティブ — リニアスケール・自動フェイルオーバー",
  ],
  "整合性": [
    "1: 最終整合のみ — 強整合保証なし",
    "2: Read-your-writes 等の限定的整合性",
    "3: 設定可能な整合性レベル（Quorum等）",
    "4: デフォルト強整合 or 単一ノードACID",
    "5: 完全ACID + 分散トランザクション（Serializable）",
  ],
  "運用容易さ": [
    "1: 設定が非常に複雑 — クラスタ必須・専門的チューニング必要",
    "2: 初期セットアップが複雑 — 専門知識が必要",
    "3: 標準設定で動作 — 学習コスト中程度",
    "4: 設定少・監視充実 — バックアップ・リストアが容易",
    "5: ゼロコンフィグ or 組込DB — インストール不要・即使用可能",
  ],
};
